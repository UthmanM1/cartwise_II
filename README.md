...
- **Fonts (Inter, Fraunces, JetBrains Mono) are self-hosted** via `@fontsource-variable/*` packages rather than `next/font/google`. The font files ship inside `node_modules` and are bundled at build time, so the build has no outbound-network dependency on Google Fonts — it succeeds the same way on Vercel, in restricted CI environments, and offline. Visually this is identical to the previous Google Fonts setup (same families, weights, and CSS variable names).
